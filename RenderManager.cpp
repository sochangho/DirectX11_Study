...
RenderManager::RenderManager(ComPtr<ID3D11Device> device,
...
void RenderManager::RenderObjects()
{
	for (const shared_ptr<GameObject>& gameObject : _renderObjects) {
		...
		//SRT
		_transformData.matWorld = transform->GetWordMatrix();
		PushTransformData();
		...
		if (animator) 
		{
			const Keyframe& keyframe = animator->GetCurrentKeyfram();
			_animationData.spriteOffset = keyframe.offset;
			_animationData.spriteSize = keyframe.size;
			_animationData.textureSize = animator->GetCurrentAnimation()->GetTexture()->GetSize();
			_animationData.useAniamtion = 1.f;
			
			PushAnimationData();

			_pipeline->
				SetConstantBuffer(2, SS_VertexShader, _animationBuffer);
			_pipeline->
				SetTexture(0, SS_PixelShader, animator->GetCurrentAnimation()->GetTexture());
		}
		else {
			...
			_animationData.useAniamtion = 0.f;
			 ...
			 _pipeline->
				 SetConstantBuffer(2, SS_VertexShader, _animationBuffer);
			 _pipeline->
				 SetTexture(0, SS_PixelShader, meshRenderer->GetTexture());
